...
protected:
	...
	auto conv = convolution_forward(conv_pd, c_src, c_weights, c_bias, c_dst);
        
        std::vector<primitive> pipeline;
        pipeline.push_back(conv);

	auto relu_dst_memory = memory(conv_pd.dst_primitive_desc());
	auto relu_desc = eltwise_forward::desc(prop_kind::forward, 
	      algorithm::eltwise_relu, conv_pd.dst_primitive_desc().desc(), negative_slope);
	auto relu_pd = eltwise_forward::primitive_desc(relu_desc, eng);
	auto relu = eltwise_forward(relu_pd, c_dst, relu_dst_memory);
