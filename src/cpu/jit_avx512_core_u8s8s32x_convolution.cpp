...
struct jit_avx512_core_u8s8s32x_conv_fwd_ker_t: public jit_generator {
    ...
    int ow_step_idx;  // conv1x1 output width [ur_ow_nsteps][ur_ow]+ow_tail, ow_step_idx~[0,ur_ow_nsteps)*ur_ow
...
void jit_avx512_core_u8s8s32x_conv_fwd_ker_t::compute(
	...
	//printf("VNNI_code");
        ...
        // compute_time.push_back(I_total);
        //printf("The submit time  = %f ms\n", I_total * 1000);
        //++c_.compute_count;
        //c_.compute_time += I_total;
}
...
#else
    ws_per_thread_ = conf_.jcp_.ow * conf_.jcp_.oc;
    ws_ = (acc_data_t *)malloc(
            nthreads * ws_per_thread_ * sizeof(acc_data_t), 64);
    
    const int oc_conv11 = 96;
    ws_per_thread_conv11 = conf_.jcp_.ow * oc_conv11;
...
execute_forward() {
    ...
    auto ker = [&](int ithr, int nthr) {
        ...
        jit_avx512_core_u8s8s32x_conv_fwd_ker_t::call_params_t p = {};
        p.acc_s32 = ws_ + ithr * ws_per_thread_;
        p.acc_conv11 = ws_conv11_memory + ithr * ws_per_thread_conv11;
#endif
